(() => {
  ...
  const renderTeams = (form) => {
    ...
    if (!panels) return;
    const exactTeams = form.teams.slice(0, 3).map((team) => `<div class="seo-team-panel"><header><strong>${team.title}</strong><span>${team.label}</span></header><div class="seo-team-strip">${team.members.map(([slug, role]) => `<span><img src="${characterImage(slug)}" alt="" loading="lazy" decoding="async"><small>${role}</small><strong>${names[slug] || slug}</strong><em>${slug === "rover" ? `${form.element} form` : "Team member"}</em></span>`).join("")}</div><p class="seo-team-note">${team.note}</p></div>`).join("");
    const partners = [...new Map(form.teams.flatMap((team) => team.members).filter(([slug]) => slug !== "rover").map(([slug, role]) => [slug, [slug, role]])).values()];
    const compatibility = `<div class="seo-team-panel seo-compatibility-panel"><header><strong>Other compatible teammates</strong><span>${form.element} Rover</span></header><p class="seo-compatibility-intro">These characters appear in the reviewed teams for the selected Rover form.</p><div class="seo-compatibility-groups"><section class="seo-compatibility-group"><div class="seo-compatibility-heading"><strong>Form-specific partners</strong><small>Use the complete teams above to keep the roles together.</small></div><div class="seo-compatibility-list">${partners.map(([slug, role]) => `<a class="seo-compatibility-person" href="../${slug}/"><img src="${characterImage(slug)}" alt="" loading="lazy" decoding="async"><span><strong>${names[slug] || slug}</strong><small>${role}</small></span></a>`).join("")}</div></section></div><p class="seo-team-note seo-compatibility-warning">Rover's forms use different teams. Changing form updates this list and does not change the selected artwork gender.</p></div>`;
    panels.innerHTML = exactTeams + compatibility;
  };
